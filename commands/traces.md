...
for arg in args:
    if arg.startswith('--operation='):
        operation_filter = arg.split('=', 1)[1]
...
if not spans:
    ...
    print('## Recent Traces\n')
    print(f'Showing {len(spans)} trace(s)' + (f' (filtered)' if operation_filter or status_filter else ''))
    print()
    ...
    for span in spans:
        ...
        status = '✓' if span.status == 'ok' else '✗' if span.status == 'error' else '○'
